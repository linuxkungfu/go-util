...
func GetFlag(countryCode string) (string, string) {
	code := strings.ToUpper(countryCode)
	if len(code) == 3 {
		if cc, ok := countryMap[code]; ok {
			emoji := string(0x1F1E6+rune(cc[0])-'A') + string(0x1F1E6+rune(cc[1])-'A')
			return emoji, GetFlagUnicode(emoji)
		}
	}
	...
	if len(code) == 2 {
		for _, v := range countryMap {
			if code == v {
				emoji := string(0x1F1E6+rune(code[0])-'A') + string(0x1F1E6+rune(code[1])-'A')
				return emoji, GetFlagUnicode(emoji)
			}
		}
	}
